...
get_crosscombat <- function(forcombat, from = "Ventricles", to = "Brainstem", is_dti = "no"){
  ...
  mod <- forcombat
  mod$Intercept <- 1
  mod <- mod %>% ungroup %>% select(Intercept, Age_at_this_scan, Sex) %>% as.matrix()
  
  #use neuroCombat
  result <- neuroCombat(dat = dat, batch = batch, mod = mod, verbose = FALSE, mean.only = FALSE)
  if (is_dti == "no"){
...
get_longcombat <- function(forcombat, is_corthick = "no", is_dti = "no"){
  ...
  df <- forcombat %>% select(Master_subject_ID,Time, Scanner, Age_at_this_scan, Sex, rois)
  ...
  longres <- longCombat(
    ...
    batchvar = "Scanner",
    features = rois,
    formula = "Age_at_this_scan + Sex + Time",
